...
import TwoPOV from './pages/TwoPOV';
...
const Navigation = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['home', 'story', 'due-punti-di-vista', 'gallery', 'video-matrimonio'];
  ...
  const navItems = [
    ...
    { id: 'due-punti-di-vista', label: 'La Nostra Storia' },
    { id: 'gallery',            label: 'Memorie' },
    { id: 'video-matrimonio',   label: 'Il Video' },
  ];
    ...
    <>
      <nav className="hidden md:block fixed top-0 w-full z-50 px-6 py-6 bg-background/90 backdrop-blur-sm border-b border-primary/10">
        <div className="max-w-5xl mx-auto relative flex justify-between items-center overflow-x-auto no-scrollbar">
          {/* The "Thread" Line */}
          <div className="absolute top-1/2 left-0 w-full h-[1px] bg-primary/40 -z-10 transform -translate-y-1/2 min-w-[700px]" />
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`md:hidden fixed inset-0 z-40 bg-background/95 backdrop-blur-xl transition-transform duration-300 ease-in-out pt-24 ${
      ...
      >
        <div className="flex flex-col items-center space-y-8 p-8 h-full overflow-y-auto">
          ...
          <div className="w-16 h-[1px] bg-primary/40 my-8" />
        </div>
      </div>
    </>
  );
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-background text-gray-800 font-serif selection:bg-primary/20">
      ...
      <section id="due-punti-di-vista" className="min-h-screen">
        <TwoPOV />
      </section>
      ...
      <section id="video-matrimonio" className="min-h-screen">
        <WeddingVideo />
      </section>
